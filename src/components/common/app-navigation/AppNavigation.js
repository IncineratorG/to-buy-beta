...
const AppNavigation = ({initialNavigationCommands}) => {
  ...
  useEffect(() => {
    if (
      navigationReady &&
      initialNavigationCommands &&
      initialNavigationCommands.length > 0
    ) {
      initialNavigationCommands.forEach((command) => {
        command.execute({navigation: navigationRef.current, dispatch});
      });
    }
  }, [navigationReady, initialNavigationCommands, dispatch]);
